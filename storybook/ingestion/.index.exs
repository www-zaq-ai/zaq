defmodule Storybook.Ingestion do
  ...
  def entry("ingestion_no_source_notice"),
    do: [name: "Ingestion No Source Notice", index: 8]

  def entry("modal_add_raw"),
    do: [name: "Modal Add Raw", index: 9]

  def entry("modal_delete"),
    do: [name: "Modal Delete", index: 10]

  def entry("modal_delete_selected"),
    do: [name: "Modal Delete Selected", index: 11]

  def entry("modal_move"),
    do: [name: "Modal Move", index: 12]

  def entry("modal_new_folder"),
    do: [name: "Modal New Folder", index: 13]

  def entry("modal_rename"),
    do: [name: "Modal Rename", index: 14]

  def entry("modal_share"),
    do: [name: "Modal Share", index: 15]

  def entry("modal_upload"),
    do: [name: "Modal Upload", index: 16]
