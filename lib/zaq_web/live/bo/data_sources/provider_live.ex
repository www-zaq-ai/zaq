defmodule ZaqWeb.Live.BO.DataSources.ProviderLive do
  ...
  @doc """
  Reports whether a root folder is bound, or `nil` when the provider has no such notion.

  Only disk answers a `"bound"` attribute — a volume can be configured while its path is not
  actually there. Every other provider returns `nil` here and renders no badge.
  """
  def folder_bound_state(%{attributes: %{"bound" => bound}}) when is_boolean(bound), do: bound
  def folder_bound_state(_folder), do: nil
