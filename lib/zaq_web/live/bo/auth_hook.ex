defmodule ZaqWeb.Live.BO.AuthHook do
  ...
  defp setup_addons_hook(socket, user) do
    ...
    |> attach_hook(:addons_updated, :handle_info, &handle_addons_updated/2)
  end

  defp handle_addons_updated(:addons_updated, socket) do
    {:cont, assign(socket, :features_version, socket.assigns.features_version + 1)}
  end

  defp handle_addons_updated(_msg, socket) do
    {:cont, socket}
  end
end
