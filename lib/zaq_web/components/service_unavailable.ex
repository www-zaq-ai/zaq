defmodule ZaqWeb.Components.ServiceUnavailable do
  ...
  For Storybook or static previews of the inner content only:

      <ZaqWeb.Components.ServiceUnavailable.failure_panel missing={[:channels]} />
  ...
  @doc """
  Renders the centered “service unavailable” body (icon, copy, missing roles, hint).

  Use with a fixed `missing` list in Storybook or docs. Production pages should use
  `page/1`, which computes `missing` from `services` via `missing_roles/1`.
  """
  attr :missing, :list, required: true

  def failure_panel(assigns) do
    ~H"""
  ...
  ## Attributes
    ...
    * `portal_consent_live_enabled` — forwarded to BOLayout (set `false` in Storybook to skip `PortalConsentLive`)
  """
  attr :current_user, :any, required: true
  attr :current_path, :string, required: true
  attr :page_title, :string, required: true
  attr :services, :list, required: true
  attr :portal_consent_live_enabled, :boolean, default: true
  ...
  def page(assigns) do
    ...
    <ZaqWeb.Components.BOLayout.bo_layout
      ...
      page_title={@page_title}
      current_path={@current_path}
      portal_consent_live_enabled={@portal_consent_live_enabled}
    >
      <.failure_panel missing={@missing} />
    </ZaqWeb.Components.BOLayout.bo_layout>
    """
  end
