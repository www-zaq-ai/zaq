defmodule Zaq.Engine.PeopleConversations do
  ...
  alias Zaq.Ingestion.Events, as: IngestionEvents
  alias Zaq.Repo
  ...
  defp conversation_operation(:source, request, auth, conversation) do
    with message when not is_nil(message) <-
         ...
         true <- Enum.any?(message.sources || [], &(source_path(&1) == source)),
         {:ok, resource} <- source_request(:read, source, auth.person.id) do
      {:ok, Map.put(resource, :actor, %{person: %{id: auth.person.id}})}
    else
      _ -> {:error, :not_found}
    end
  end
  ...
  defp conversation_operation(:artifact, request, auth, conversation) do
    with message when not is_nil(message) <-
         ...
         true <- trace_references?(message.trace, artifact.id),
         :ok <- artifact_access(artifact.record, auth.person.id) do
      {:ok, Map.take(artifact, [:content, :name, :mime_type])}
    else
      _ -> {:error, :not_found}
    end
  end
  ...
  defp source_request(op, source, id) do
    IngestionEvents.build_and_dispatch_source_content_event(%{op: op, source: source},
      actor: %{person: %{id: id}},
      event_opts: [confidential: true]
    ).response
  end

  # Communication snapshots are private children of the already-authorized
  # message. Source-backed artifacts additionally retain their document ACL.
  # Unattributed historical resources cannot establish a current resource grant.
  defp artifact_access(%{"attributes" => %{"source" => source}}, id),
    do: source_request(:authorize, source, id)

  defp artifact_access(%{"path" => source}, id) when is_binary(source),
    do: source_request(:authorize, source, id)

  defp artifact_access(%{"attributes" => %{"source_type" => "communication_media"}}, _), do: :ok

  defp artifact_access(_, _), do: {:error, :not_found}
