defmodule Zaq.Engine.PeopleAuthGateway do
  ...
  def dispatch(request, opts) do
    case request do
      %{op: :request_challenge, email: email, ip: ip} -> request_challenge(email, ip, opts)
      %{op: :verify, challenge_id: id, code: code} -> PeopleAuth.verify_challenge(id, code, opts)
      %{op: :authenticate, token: token} -> PeopleAuth.authenticate(token, opts)
      %{op: :revoke, token: token} -> PeopleAuth.revoke_session(token)
  ...
  defp send_challenge(person_id, challenge, opts) do
    ...
    params = %{
      ...
      message: "Your ZAQ sign-in code is #{first}-#{last}. Do not share this code."
    }
