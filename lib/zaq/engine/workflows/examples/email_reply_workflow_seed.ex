defmodule Zaq.Engine.Workflows.Examples.EmailReplyWorkflowSeed do
  ...
  ## Usage
      ...
      # Fire a manual run
      {:ok, run} = Zaq.Engine.Workflows.Examples.EmailReplyWorkflowSeed.fire(workflow)
  ...
  3. `fire/1` delegates to `Triggers.Manual.fire_for_workflow/2`, which builds a
  ...
  def seed do
    case Repo.get_by(Workflow, name: @workflow_name) do
      %Workflow{} = existing ->
        with {:ok, workflow} <-
               Workflows.update_workflow(existing, %{nodes: @nodes, edges: @edges}) do
          {:ok, workflow, ensure_trigger(workflow)}
        end

      nil ->
        with {:ok, workflow} <- Workflows.create_workflow(workflow_attrs()),
             {:ok, trigger} <- Workflows.create_trigger(%{type: "manual", enabled: true}),
             {:ok, _} <- Workflows.assign_workflow_to_trigger(trigger, workflow) do
          {:ok, workflow, trigger}
        end
    end
  end

  defp ensure_trigger(workflow) do
    case Workflows.list_triggers_for_workflow(workflow.id) do
      [trigger | _] ->
        trigger
  ...
  @doc """
  Fires a manual run for the given workflow.

  Returns `{:ok, run}` where `run.status` is `"pending"` and
  `run.steps_snapshot` holds the snapshotted DAG.
  """
  @spec fire(Workflow.t()) :: {:ok, term()} | {:error, term()}
  def fire(%Workflow{} = workflow) do
    Manual.fire_for_workflow(workflow, %{mailbox: "INBOX"})
  end
  ...
  def seed_and_fire do
    with {:ok, workflow, _trigger} <- seed() do
      fire(workflow)
    end
  end
  ...
  def validate_dag do
    DagBuilder.build(%{
      "nodes" => Enum.map(@nodes, &stringify_node/1),
      "edges" => Enum.map(@edges, fn e -> %{"from" => e.from, "to" => e.to} end)
    })
  end
  ...
  def run(mailbox \\ nil) do
    ...
    with {:ok, workflow, _trigger} <- seed(),
         {:ok, run} <-
           Manual.fire_for_workflow(workflow, %{
  ...
  def triggers do
    case workflow() do
      nil -> []
      wf -> Workflows.list_triggers_for_workflow(wf.id)
    end
  end
  ...
  def inspect_all do
    ...
    case wf do
      ...
      _ ->
        ...
        IO.puts("  nodes:  #{length(wf.nodes)}")
        IO.puts("  edges:  #{length(wf.edges)}")
    end
  ...
  defp stringify_node(n) do
    %{
      "name" => n.name,
      "type" => n.type,
      "module" => Map.get(n, :module),
      "params" => Map.get(n, :params, %{}),
      "index" => n.index
    }
  end
end
