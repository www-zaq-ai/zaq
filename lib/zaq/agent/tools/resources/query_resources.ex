defmodule Zaq.Agent.Tools.Resources.QueryResources do
  ...
  `person`, `ai_provider`, and `incoming_message_routing_rule`. Use `mode: "describe"` to discover each resource's
  ...
  @resource_names ~w(agent mcp skill user person ai_provider incoming_message_routing_rule)
  ...
  @schema Zoi.object(%{
            ...
            resource_type:
              Zoi.enum(@resource_names,
                description:
                  "Resource type. Allowed: agent, mcp, skill, user, person, ai_provider, incoming_message_routing_rule. Optional only for mode=describe."
              )
              |> Zoi.optional(),
            id:
