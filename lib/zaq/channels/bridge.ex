defmodule Zaq.Channels.Bridge do
  ...
  def list_providers(kind, opts \\ []) when kind in [:communication, :data_source] do
    %{config_kind: config_kind, capability: {fun, arity}} = provider_spec(kind)
    ...
    providers =
      ...
      |> Enum.map(fn {provider, enabled} ->
        %{provider: provider, label: ProviderCatalog.label(provider), status: status(enabled)}
      end)
  ...
  defp provider_spec(:communication),
    do: %{config_kind: :retrieval, capability: {:send_reply, 2}}

  defp provider_spec(:data_source),
    do: %{config_kind: :data_source, capability: {:create_file, 2}}
