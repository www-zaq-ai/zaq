    ...
    BEGIN
      IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'ZAQ requires vector >= 0.7.0 with halfvec visible on search_path'
          USING HINT = 'For local development, run mix db.extensions with an extension-capable configured database user. In managed environments, a DBA must bootstrap or repair extensions in this same database using scripts/setup_postgres_extensions.sql (PostgreSQL) or scripts/setup_paradedb_extensions.sql (ParadeDB). Older extensions require a DBA-managed upgrade; also check search_path.';
      END IF;
    END;
    $$
    """
    ...
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_extension WHERE extname = 'pg_search') THEN
        RAISE EXCEPTION 'ZAQ ParadeDB backend requires the pg_search extension'
          USING HINT = 'For local development, run mix db.extensions with an extension-capable configured database user. In managed environments, a DBA must bootstrap this database using scripts/setup_paradedb_extensions.sql. Plain PostgreSQL uses native full-text search and scripts/setup_postgres_extensions.sql instead.';
      END IF;
    END;
    $$
    """
