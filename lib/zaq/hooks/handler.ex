defmodule Zaq.Hooks.Handler do
  ...
  how the dispatch chain proceeds:
    ...
    * `{:halt, payload}` — stop chain; `dispatch_sync` returns `{:halt, payload}`
    * `{:error, term}`   — silently skip this handler, log a warning, continue chain
    * `:ok`              — observer acknowledgement (`dispatch_async`); payload unchanged

  See `Zaq.Hooks` for the full event catalogue and payload shapes.
  """
