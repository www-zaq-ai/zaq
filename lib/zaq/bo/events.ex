defmodule Zaq.BO.Events do
  @moduledoc """
  Standardized BO role event builders and dispatchers.
  """
  ...
  def build_invoke_event(request, action, opts \\ []) when is_map(request) and is_atom(action) do
    Helper.build_invoke_event(:bo, request, action, opts)
  end
  ...
  def build_and_dispatch_invoke_event(request, action, opts \\ [])
    ...
    Helper.build_and_dispatch_invoke_event(:bo, request, action, opts)
  end
end
