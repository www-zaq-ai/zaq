...
**Node types:**
- `"action"` / `"agent"` — wrapped in `Jido.Runic.ActionNode`. When `run_id` is present, wrapped further by `StepRunner`.
...
**Idempotency:** `StepRunner` checks for an existing completed `StepRun` before executing any step. Once `approve_run/5` promotes the HITL `StepRun` to `"completed"`, any subsequent resume skips it and passes the stored `%{approved: true, ...}` map to downstream steps automatically.
