...
2. Have the DBA create the target database and assign it to ZAQ's non-superuser
   login. A `NOCREATEDB` application login cannot run `mix ecto.create` itself.
   Ensure this login owns its application schemas and existing application objects.
...
Provision every development, test, E2E and worktree database separately. Have a
DBA create the database (or use `mix ecto.create` with a suitably privileged local
development login), provision the selected database as DBA, then run `mix setup` /
`mix test`. Test database names include the branch slug, and E2E uses a separate
`zaq_test_e2e_<slug>` name.
`mix ecto.reset` drops extensions with the database: instead drop and recreate the
database using an authorized login, have the DBA provision it, then run
`mix ecto.migrate`.
