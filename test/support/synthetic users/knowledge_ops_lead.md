...
- Scope: ingestion workflows, ontology modeling, prompt governance, and answer verification loops.
- Primary UI surfaces: `/bo/ingestion`, `/bo/ontology`, `/bo/prompt-templates`, `/bo/playground`, `/bo/preview/*path`.
...
### Journey 3: Model organizational context in ontology
Sequence of pages visited:
1. `/bo/ontology`

| Page visited | Elements seen | Elements interacted with |
| --- | --- | --- |
| `/bo/ontology` | Tab bar (`Tree View`, `Org Structure`, `People & Channels`, `Knowledge Domains`), hierarchy cards, add/edit/delete controls, modal form | Switch tabs, add Business -> Division -> Department -> Team from `Org Structure`, add Person and Channels in `People & Channels`, add Knowledge Domain linked to a department, use node expand/collapse controls to verify structure |

### Journey 4: Tune prompt behavior and verify answer quality loop
Sequence of pages visited:
