defmodule Zaq.Engine.Workflows.WorkflowApprovalTest do
  ...
  describe "changeset/2 — valid" do
    test "accepts valid attrs" do
      run = insert_run!()
      cs = WorkflowApproval.changeset(%WorkflowApproval{}, valid_attrs(run))
      assert cs.valid?
    end
    ...
    test "message is optional" do
      ...
      cs = WorkflowApproval.changeset(%WorkflowApproval{}, attrs)
      assert cs.valid?
    end
  ...
  describe "unique constraints" do
    test "duplicate approval_token raises constraint error" do
      ...
      {:ok, _} = Repo.insert(WorkflowApproval.changeset(%WorkflowApproval{}, attrs))

      run2 = insert_run!()
      attrs2 = %{valid_attrs(run2) | approval_token: token}
      {:error, cs} = Repo.insert(WorkflowApproval.changeset(%WorkflowApproval{}, attrs2))
  ...
  describe "statuses/0" do
    test "returns the valid status list" do
      assert WorkflowApproval.statuses() == ~w(pending approved rejected)
    end
  end
end
