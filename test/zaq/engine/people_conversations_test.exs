defmodule Zaq.Engine.PeopleConversationsTest do
  ...
  test "citations require bounded message references without an indexed document", ctx do
    grant_history()
    ...
    params = %{message_id: message.id, source: source}
    assert Document.get_by_source(source) == nil
    assert {:ok, %{kind: :source, document_reference: ^source}} = call(ctx, :source, params)
    ...
    {:ok, binary_source} =
      Conversations.add_message(ctx.own, %{
        role: "assistant",
        content: "Source",
        sources: [%{"type" => "memory"}, %{"source" => source}]
      })

    assert {:ok, %{kind: :source, document_reference: ^source}} =
  ...
  test "source references forward no bytes and use only the authenticated actor", ctx do
    ...
    assert call(ctx, :source, params) ==
             {:ok,
              %{
                kind: :source,
                document_reference: document.source,
                actor: %{person: %{id: ctx.person.id, full_name: "Reader", team_ids: []}}
              }}
  end
  ...
  test "artifact reads require the owned message and its trace root reference", ctx do
    ...
    assert {:ok, %{kind: :record, record: %{content: "bytes"}}} = call(ctx, :artifact, params)
    ...
    for record <- [
          %{
            "attributes" => %{"source" => source, "source_type" => "communication_media"}
          },
