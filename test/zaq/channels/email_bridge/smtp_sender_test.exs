defmodule Zaq.Channels.EmailBridge.SmtpSenderTest do
  ...
  describe "deliver/3" do
    test "delivers with default sender when channel config is missing" do
      assert :ok = SmtpSender.deliver("user@example.com", payload(), %{})
    ...
    test "uses email_body from metadata over payload body when present" do
      ...
      assert :ok = SmtpSender.deliver("user@example.com", payload(), metadata)
    ...
    test "uses html_body from payload when present" do
      ...
      assert :ok = SmtpSender.deliver("user@example.com", p, %{})
    ...
    test "handles empty payload gracefully" do
      ...
      assert :ok = SmtpSender.deliver("user@example.com", %{}, %{})
    ...
    test "uses ssl transport mode branch" do
      ...
      assert {:error, _reason} =
               SmtpSender.deliver("user@example.com", payload(), %{})
    end
    ...
    test "prefers metadata headers over payload and filters invalid headers" do
      ...
      assert :ok = SmtpSender.deliver("user@example.com", p, metadata)
  ...
  describe "coverage gaps for sender parsing and cert fallback" do
    ...
    test "decrypts valid encrypted password for username auth without crashing" do
      ...
      result = SmtpSender.deliver("user@example.com", payload(), %{})
