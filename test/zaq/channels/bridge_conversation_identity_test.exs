defmodule Zaq.Channels.BridgeConversationIdentityTest do
  ...
  alias Zaq.Channels.{Bridge, EmailBridge}
  ...
  describe "conversation_channel_type/2" do
    test "maps email providers to the imap conversation channel" do
      assert Bridge.conversation_channel_type("email") == "email:imap"
      assert Bridge.conversation_channel_type("email:smtp") == "email:imap"
      assert Bridge.conversation_channel_type("email:imap") == "email:imap"
      assert Bridge.conversation_channel_type(:email) == "email:imap"
      assert Bridge.conversation_channel_type(:"email:smtp") == "email:imap"
    end

    test "maps web to the bo conversation channel" do
      assert Bridge.conversation_channel_type("web") == "bo"
      assert Bridge.conversation_channel_type(:web) == "bo"
    end

    test "passes other providers through unchanged" do
      assert Bridge.conversation_channel_type("mattermost") == "mattermost"
      assert Bridge.conversation_channel_type(:mattermost) == "mattermost"
    end

    test "non-atom, non-binary providers resolve to api" do
      assert Bridge.conversation_channel_type(123) == "api"
      assert Bridge.conversation_channel_type(%{}) == "api"
    end
  end
  ...
  describe "conversation_key/3 (email channel)" do
    test "prefers the email metadata thread_key" do
      ...
      assert Bridge.conversation_key(msg, "email:imap") == "<root@mail.example>"
    end

    test "falls back to the top-level thread_key" do
      msg = incoming(%{metadata: %{"thread_key" => "outer-key", "topic" => "campaign"}})

      assert Bridge.conversation_key(msg, "email:imap") == "outer-key"
    end
    ...
    test "falls back to topic then subject for outbound-first sends" do
      ...
      assert Bridge.conversation_key(by_topic, "email:imap") == "campaign"
      assert Bridge.conversation_key(by_subject, "email:imap") == "Hello"
    end
    ...
    test "skips blank strings in the precedence chain" do
      ...
      assert Bridge.conversation_key(msg, "email:imap") == "Hello"
    end
    ...
    test "falls back to the normalized thread_id, then message_id" do
      ...
      assert Bridge.conversation_key(by_thread, "email:imap") == "t-1@mail.example"
      assert Bridge.conversation_key(by_message, "email:imap") == "m-1@mail.example"
    end

    test "returns nil when nothing resolves (caller owns the author fallback)" do
      msg = incoming(%{author_id: "person@example.com"})

      assert Bridge.conversation_key(msg, "email:imap") == nil
    end
  ...
  describe "conversation_key/3 (other channels)" do
    test "returns nil for channels without a grouping callback" do
      ...
      assert Bridge.conversation_key(msg, "mattermost") == nil
      assert Bridge.conversation_key(msg, "bo") == nil
      assert Bridge.conversation_key(msg, "api") == nil
    end
  end

  describe "outbound_conversation_key/4" do
    test "email platforms group by topic, then subject" do
      assert Bridge.outbound_conversation_key("email", "campaign", "Hello") == "campaign"
      assert Bridge.outbound_conversation_key("email:smtp", nil, "Hello") == "Hello"
      assert Bridge.outbound_conversation_key("email:imap", "campaign", nil) == "campaign"
    end

    test "blank topic falls through to subject" do
      assert Bridge.outbound_conversation_key("email", "  ", "Hello") == "Hello"
    end

    test "returns nil when both topic and subject are blank" do
      assert Bridge.outbound_conversation_key("email", nil, nil) == nil
      assert Bridge.outbound_conversation_key("email", "", " ") == nil
    end

    test "non-email platforms return nil" do
      assert Bridge.outbound_conversation_key("mattermost", "campaign", "Hello") == nil
      assert Bridge.outbound_conversation_key("bo", "campaign", "Hello") == nil
    end
  end
