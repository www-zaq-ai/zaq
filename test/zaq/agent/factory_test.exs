defmodule Zaq.Agent.FactoryTest do
  ...
  describe "build_model_spec/1 — ConfiguredAgent" do
    test "sets base_url from credential endpoint for custom OpenAI-compatible providers" do
      ...
      assert {:ok, spec} = Factory.build_model_spec(configured_agent)
      assert spec.provider == :openai
    ...
    test "returns provider_not_found error when provider unknown and no custom endpoint" do
      ...
      assert {:error, :provider_not_found} = Factory.build_model_spec(configured_agent)
    end
  end
