defmodule Zaq.Agent.Tools.KnowledgeBaseOverviewUnitTest do
  ...
  defmodule PlainListRouter do
    def dispatch(%Event{request: %{function: :list_files_with_ingestion_status}} = event),
      do: %{event | response: [%{source: "plain.md", ingested: true, title: "Plain"}]}
  end
  ...
  defmodule PermissionRouter do
    def dispatch(
          %Event{
            request: %{
              function: :list_files_with_ingestion_status,
              args: [[person_id: 42, team_ids: [1, 2], skip_permissions: false]]
            }
          } = event
        ),
        do: %{event | response: {:ok, []}}
    ...
    def dispatch(
          %Event{request: %{function: :list_files_with_ingestion_status, args: [opts]}} = event
        ),
        do: %{event | response: {:ok, [{:unexpected_opts, opts}]}}
  end
