defmodule Zaq.Agent.Tools.General.ListProvidersTest do
  ...
  defmodule StubNodeRouter do
    ...
    def dispatch(%Event{request: request, opts: opts}) do
      ...
      %{
        Event.new(%{}, :channels)
        | response: {:ok, %{providers: [%{provider: "google_drive", label: "Google Drive"}]}}
      }
    end
  end
  ...
  test "dispatches the provider listing action with the requested kind" do
    assert {:ok, %{providers: [%{provider: "google_drive", label: "Google Drive"}]}} =
  ...
  describe "through the real channels boundary" do
    ...
    test "data_source lists every writable data_source config with its status" do
      ...
      assert Enum.map(providers, & &1.provider) == ["google_drive", "sharepoint"]
      assert Enum.map(providers, & &1.label) == ["Google Drive", "SharePoint"]
      assert Enum.map(providers, & &1.status) == [:active, :inactive]
    end

    test "data_source offers nothing when no data_source config exists" do
      assert {:ok, %{providers: []}} =
               ListProviders.run(%{kind: "data_source"}, %{node_router: LocalNodeRouter})
    end

    test "communication lists retrieval configs with a status" do
      insert_config("mattermost", "retrieval", true)
