defmodule ZaqWeb.Components.ChatMessageTest do
  ...
  # ── Group 2 — tool_calls_popin with empty expanded_ids (collapsed default) ───

  test "tool_calls_popin with empty expanded_ids renders row collapsed" do
    html =
      render_component(&ChatMessage.tool_calls_popin/1,
        visible: true,
        message_id: "msg-x",
        tool_calls: [%{"tool_call_id" => "c1", "tool_name" => "search"}],
        expanded_ids: MapSet.new(),
        close_event: "close",
        toggle_event: "toggle"
      )
    ...
    assert html =~ "data-testid=\"tool-call-row-c1\""
    refute html =~ "tool-call-details"
  end

  # ── Group 3 — format_detail_value/1 with nil and integer timestamps ─────────

  test "tool_calls_popin expanded shows n/a for nil timestamp and inspect for integer" do
    nil_html =
      render_component(&ChatMessage.tool_calls_popin/1,
        visible: true,
        message_id: "msg-nil-ts",
        tool_calls: [
          %{
            "tool_call_id" => "c-nil",
            "tool_name" => "search",
        ...
        expanded_ids: MapSet.new(["c-nil"]),
        close_event: "close_tool_calls_modal",
        toggle_event: "toggle_tool_call_details"
      )

    assert nil_html =~ "Timestamp:</span>\n              n/a"

    int_html =
      render_component(&ChatMessage.tool_calls_popin/1,
        visible: true,
        message_id: "msg-int-ts",
        tool_calls: [
          %{
            "tool_call_id" => "c-int",
            "tool_name" => "search",
        ...
        expanded_ids: MapSet.new(["c-int"]),
        close_event: "close_tool_calls_modal",
        toggle_event: "toggle_tool_call_details"
      )

    assert int_html =~ "12345"
  end
