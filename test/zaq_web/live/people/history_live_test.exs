defmodule ZaqWeb.Live.People.HistoryLiveTest do
  ...
  test "local source types retain safe preview and download URLs", ctx do
    for {name, bytes, selector} <- [
      ...
      document = PeopleSourceFixture.create(ctx.person, bytes, name)
  ...
  test "source previews and byte routes enforce message parent and document permission", ctx do
    document = PeopleSourceFixture.create(ctx.person, "Authorized citation")
    :ok = Permissions.revoke(nil, document.grant)
    ...
    {:ok, _} =
      Storage.grant_document_access(document.entry.id, %{
        person_id: ctx.person.id,
        access_rights: ["read"]
      })
