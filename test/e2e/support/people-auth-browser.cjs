...
async function main() {
  ...
  try {
    ...
    await expect(page.getByText("This profile is read-only.", { exact: false })).toBeVisible()
    ...
    const priority = page.getByLabel(`Priority for email / ${email}`, { exact: true })
    await priority.fill("5")
    await page.getByRole("button", { name: `Save priority for email / ${email}`, exact: true }).click()
    await expect(page.getByRole("status")).toContainText("Priority saved")
    await expect(page.locator('[id^="profile-channel-"] h3').first()).toHaveText("slack")
    await page.reload()
    await waitForLiveViewConnected(page)
    await expect(page.getByLabel("Full name", { exact: true })).toHaveValue("Browser Person Updated")
    ...
    await expect(page.locator("#self-profile-form")).toHaveCount(0)
    await expect(page.getByText("Welcome, Browser Person Updated.")).toBeVisible()
    await expect(page.getByText("Priority: 5", { exact: true })).toBeVisible()
    await admin.close()
